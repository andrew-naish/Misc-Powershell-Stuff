...
"@
Add-Type -Language CSharp -TypeDefinition $LSPRightsCS
...
function Test-LogonAsBatchRight {
    param(
    ...
    # check is admin
    if ( -not (Test-IsAdmin) ) 
    { throw "This function requires that you have admin privileges" }
    

    try {

        $result = @( [LSA.LSPWrapper]::GetRights("$UserId") )

        if ($result.Contains('SeBatchLogonRight')) {
            return $true
        }

    } catch {} # don't care about errors will just return false

    return $false 

}

function Add-LogonAsBatchRight {
    param(
        [Parameter(Mandatory=$true)]
        [String] $UserId
    )
    ...
    if ( Test-LogonAsBatchRight $UserId ) { return $true }

    try {

        [LSA.LSPWrapper]::SetRight("$UserId", 'SeBatchLogonRight')
        return $true
